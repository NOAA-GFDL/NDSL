...
@pytest.fixture
def local_communicator_list(total_ranks: int) -> list[LocalComm]:
    shared_buffer: dict = {}
    return_list: list = []
...
def test_local_comm_simple(local_communicator_list: list[LocalComm]) -> None:
...
def test_local_comm_tags(
    local_communicator_list: list[LocalComm], tags: tuple[int, int, int]
) -> None:
    for comm in local_communicator_list:
        rank = comm.Get_rank()
        size = comm.Get_size()
