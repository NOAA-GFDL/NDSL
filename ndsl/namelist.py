...
# to submodules.
# NmlDefaults and Nml are proposed scaled down versions of NamelistDefaults and
# Namelist to facilitate this reduction. Eventually, these NmlDefaults
# and Nml classes may be removed entirely. For now, they will be used as an
# intermediary step to help remove the dependencies of Namelist to PyFV3 and
# PySHiELD submodules.
class NmlDefaults:
    dt_atmos: int = DEFAULT_INT
    npx: int = DEFAULT_INT
...
@dataclasses.dataclass
class Nml(f90nml.Namelist):
    dt_atmos: int = NmlDefaults.dt_atmos
    npx: int = NmlDefaults.npx
    npy: int = NmlDefaults.npy
    npz: int = NmlDefaults.npz
    layout: Tuple[int, int] = NmlDefaults.layout
